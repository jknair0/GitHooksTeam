...
open class GitHooksTeamPlugin : Plugin<Project> {
    ...
    override fun apply(project: Project) {
        val extension = project.extensions.create<GitHooksTeamExtension>(PLUGIN_NAME, GitHooksTeamExtension::class.java)
        val srcDir = extension.srcDir ?: getDefaultSrcDir(project)
        val destDir = getHooksDir(project)
        project.tasks.create<Copy>(TASK_NAME, Copy::class.java) {
            from(srcDir)
            into(destDir)
            doLast {
    ...
    private fun createAddFileExecPermissionTask(project: Project, dirPath: String): Exec {
        return project.tasks.create<Exec>("addExecutePermission", Exec::class.java) {
